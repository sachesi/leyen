...
pub fn animate_scroll_to_top(toolbar_view: &adw::ToolbarView) {
    ...
    glib::spawn_future_local(async move {
        // Yield to let GTK finish laying out new content
        glib::timeout_future(std::time::Duration::from_millis(33)).await;

        let start = std::time::Instant::now();
        let duration = std::time::Duration::from_millis(200).as_secs_f64();
        loop {
